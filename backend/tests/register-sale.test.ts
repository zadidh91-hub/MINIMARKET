...
function product(partial: Partial<Product> & Pick<Product, "id" | "name" | "code" | "price" | "stock">): Product {
  const now = new Date();
  return {
    minStock: 5,
    imagePath: null,
...
class InMemoryProducts implements ProductRepository {
  ...
  async update(p: Product) {
    this.items = this.items.map((i) => (i.id === p.id ? p : i));
    return p;
  }
...
class FakeSaleWriter implements SaleRegistrationWriter {
  ...
  async register(data: Parameters<SaleRegistrationWriter["register"]>[0]): Promise<Sale> {
    this.last = data;
    return data.sale;
  }
}
...
describe("RegisterSaleUseCase", () => {
  const rice = product({ id: "p1", name: "Arroz", code: "ARR001", price: 4.5, stock: 20 });
  const oil = product({ id: "p2", name: "Aceite", code: "ACE001", price: 10, stock: 5 });
  ...
  it("calcula totales con precios del dominio y descuenta stock", async () => {
    ...
    expect(writer.last?.products.find((p) => p.id === "p1")?.stock).toBe(18);
    expect(writer.last?.products.find((p) => p.id === "p2")?.stock).toBe(4);
